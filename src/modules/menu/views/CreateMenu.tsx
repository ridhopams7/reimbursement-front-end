...
class CreateMenu extends Component<any, any> {
    ...
    componentDidMount = async () => {
        try {
            ...
            // const menuTypeId = this.props.match.params ? this.props.match.params.menuTypeId : null;
            const { headers } = this.state;
            let menu = voucherRoles.V_Entry;
            // let permission = null;
    ...
    initialLoadCreateTransaction = async () => {
        try {
            const copyDataMenuData = { ...defaultMenuData };
            this.setState({
                title: "Create Menu",
    ...
    handleRemoveTransaction = async () => {
        try {
            const { headers } = this.state;
            const userRoles = this.props.resAuth.menus.find((obj: any) => obj.menu === voucherRoles.V_Remove);
            const token: any = { ...headers };
            const copyheaders = {
                Authorization: token.Authorization,
                menu: voucherRoles.V_Remove,
    ...
    handleSaveTransaction = async () => {
        try {
            ...
            // const userRoles = this.props.resAuth.roles.find((obj: any) => obj.roles === role);
            ...
            HttpService.post('menu', formData, headersmultipart)
                .then((res: any) => {
...
export default connect(mapStateToProps, null)(CreateMenu);
