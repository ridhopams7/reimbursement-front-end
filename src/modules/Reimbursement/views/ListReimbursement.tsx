...
import {
  initialPage,
  defaultRequest, defaultFilter, defaultReimbursementList, REIMBURSEMENTLISTENDPOINT,
...
class ListReimbursement extends Component<any, any> {
  ...
  componentDidMount = async () => {
    try {
      const userRoles = this.props.resAuth.menus.find((obj: any) =>
        obj.menu === reimbursementTransactionRole.Reimbursement_Transaction_View);
      if (!userRoles) {
  ...
  initialLoadViewSummary = async () => {
    try {
      ...
      const copyheaders = {
        ...
        menu: reimbursementTransactionRole.Reimbursement_Transaction_View,
      ...
      await this.getReimbursements(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, copyheaders);
  ...
  getReimbursements = async (filterData: any, orderBy: string, direction: string, page: number, pageSize: number, headers: any) => {
    try {
      const data = {
        ...filterData,
        orderBy,
      ...
      const res = await HttpService.post(REIMBURSEMENTLISTENDPOINT, data, headers);
  ...
  handleOnClear = () => {
    try {
      ...
      this.getReimbursements(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, headers);
    } catch (e) {
  ...
  handleChangePage = (page: number) => {
    try {
      ...
      this.setState(request);
      this.getReimbursements(data, order, direction, page - 1, pageSize, headers);
    } catch (e) {
  ...
  handlePageSizeGo = (e: any) => {
    try {
      ...
      this.getReimbursements(data, order, direction, currentPage, pageSize, headers);
...
export default connect(mapStateToProps)(ListReimbursement);
