...
import {
  initialPage, 
  defaultRequest, defaultFilter, defaultUserList, USERENDPOINT,
...
class ListUser extends Component<any, any> {
  ...
  componentDidMount = async () => {
    try {
      const userRoles = this.props.resAuth.menus.find((obj: any) => obj.menu === userUser.User_Apps_View);
      if (!userRoles) {
  ...
  initialLoadViewSummary = async () => {
    try {
      ...
      await this.getUserDatas(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, copyheaders);
  ...
  setSweetAlert = (param: any, userId: string) => {
    let onConfirm = null;

    onConfirm = () => { this.handleRemoveTransaction(userId) };
  ...
  handleRemoveTransaction = async (userId: string) => {
    try {
      const { headers } = this.state;
      ...
      await HttpService.delete(`user/${userId}`, null, copyheaders)
        .then((res: any) => {
          ...
          let onConfirm = async () => {
            this.hideSweetAlert();
            await this.getUserDatas(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, null);
          };
          let onCancel = () => {
            this.hideSweetAlert();
          };
  ...
  getUserDatas = async (filterData: any, orderBy: string, direction: string, page: number, pageSize: number, headers: any) => {
    try {
      const data = {
        ...filterData,
        orderBy,
      ...
      const res = await HttpService.post(USERENDPOINT, data, headers);
      //  console.log(res);
      const { request } = this.state;
      const pagenumber: any = request.currentPage === 0 ? 1 : request.currentPage;
      
      const users = { ...defaultUserList };
      let errorMessage = "";
      if (res.data) {
      ...
      this.setState({ res: users, alertMessage: errorMessage, isAlert: true }, () => {
  ...
  handleOnClear = () => {
    try {
      ...
      this.getUserDatas(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, headers);
    } catch (e) {
  ...
  handleChangePage = (page: number) => {
    try {
      ...
      this.setState(request);
      this.getUserDatas(data, order, direction, page - 1, pageSize, headers);
    } catch (e) {
  ...
  handlePageSizeGo = (e: any) => {
    try {
      ...
      this.getUserDatas(data, order, direction, currentPage, pageSize, headers);
  ...
  render() {
    ...
    return (
      ...
      setSweetAlert={(param: any, userId: string) => this.setSweetAlert(param, userId)}
