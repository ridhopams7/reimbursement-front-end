...
import {
  initialPage, 
  defaultRequest, defaultFilter, defaultMasterList, MASTERTYPESENDPOINT,
...
class ListMaster extends Component<any, any> {
  ...
  componentDidMount = async () => {
    try {
      const userRoles = this.props.resAuth.menus.find((obj: any) => obj.menu === const_masterType.Master_Type_View);
      if (!userRoles) {
  ...
  initialLoadViewSummary = async () => {
    try {
      ...
      // const userRoles = this.props.resAuth.menus.find((obj: any) => obj.role === masterType.Master_Type_View);
      const copyheaders = {
        ...headers,
        role: const_masterType.Master_Type_View,
      ...
      await this.getMasterTypes(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, copyheaders);
  ...
  getMasterTypes = async (filterData: any, orderBy: string, direction: string, page: number, pageSize: number, headers: any) => {
    try {
      const data = {
        ...filterData,
        orderBy,
      ...
      const res = await HttpService.post(MASTERTYPESENDPOINT, data, headers);
  ...
  handleOnClear = () => {
    try {
      ...
      this.getMasterTypes(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, headers);
    } catch (e) {
  ...
  handleChangePage = (page: number) => {
    try {
      ...
      this.setState(request);
      this.getMasterTypes(data, order, direction, page - 1, pageSize, headers);
    } catch (e) {
  ...
  handlePageSizeGo = (e: any) => {
    try {
      ...
      this.getMasterTypes(data, order, direction, currentPage, pageSize, headers);
