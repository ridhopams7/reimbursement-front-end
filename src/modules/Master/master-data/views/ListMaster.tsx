...
import {
  initialPage,
  defaultRequest, defaultFilter, defaultMasterList, MASTERDATASENDPOINT, defaultCombobox,
...
class ListMaster extends Component<any, any> {
  constructor(props: any) {
    super(props);
    this.state = {
      action: '',
      err: '',
      res: { ...defaultMasterList },
  ...
  componentDidMount = async () => {
    try {
      const userRoles = this.props.resAuth.menus.find((obj: any) => obj.menu === const_masterData.Master_Data_View);
      if (!userRoles) {
  ...
  initialLoadViewSummary = async () => {
    try {
      ...
      const copyheaders = {
        ...headers,
        role: const_masterData.Master_Data_View,
      ...
      await this.getMasterDatas(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, copyheaders);

      this.setState({ filter, headers: copyheaders, disablecombobox, combobox: copyCombobox });
    } catch (e) {
  ...
  getMasterDatas = async (filterData: any, orderBy: string, direction: string, page: number, pageSize: number, headers: any) => {
    try {
      const data = {
        ...filterData,
        orderBy,
      ...
      const res = await HttpService.post(MASTERDATASENDPOINT, data, headers);
      //  console.log(res);
      const { request } = this.state;
      const pagenumber: any = request.currentPage === 0 ? 1 : request.currentPage;

      const masters = { ...defaultMasterList };
      let errorMessage = "";
      if (res.data) {
  ...
  handleOnClear = () => {
    try {
      ...
      this.getMasterDatas(data, initialPage.order, initialPage.direction, initialPage.pagenumber, initialPage.pageSize, headers);
    } catch (e) {
  ...
  handleChangePage = (page: number) => {
    try {
      ...
      this.setState(request);
      this.getMasterDatas(data, order, direction, page - 1, pageSize, headers);
    } catch (e) {
  ...
  handlePageSizeGo = (e: any) => {
    try {
      ...
      this.getMasterDatas(data, order, direction, currentPage, pageSize, headers);
  ...
  render() {
    const { res, request, filter, sweetAlert, combobox } = this.state;
    ...
    return (
      <div className="animated fadeIn">
        ...
        {sweetAlert}
        <Search filter={filter} history={this.props.history} combobox={combobox}
